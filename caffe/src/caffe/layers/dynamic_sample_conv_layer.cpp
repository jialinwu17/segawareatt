...
void DynamicSampleConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
 ...
 // printf("haha\n");
  ...
  lr_mul = conv_param.lr_mul();
  //printf("haha\n");
  
}

template <typename Dtype>
void DynamicSampleConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int first_spatial_axis = channel_axis_ + 1;
  ...
  top_shape.push_back(samples_*samples_*num_output_);
...
template <typename Dtype>
void DynamicSampleConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void DynamicSampleConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void DynamicSampleConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(DynamicSampleConvolutionLayer);
#endif

INSTANTIATE_CLASS(DynamicSampleConvolutionLayer);
REGISTER_LAYER_CLASS(DynamicSampleConvolution);
